  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      const tile = {
        ...
        enemyData: st.enemyData ? structuredClone(st.enemyData) : null,
        itemData: st.itemData ? structuredClone(st.itemData) : null,
        chestLoot: st.chestLoot ? structuredClone(st.chestLoot) : null,
        chestReady: st.chestReady,
        chestLooted: st.chestLooted,
        magicChestReady: st.magicChestReady,
        pendingLoot: st.pendingLoot ? structuredClone(st.pendingLoot) : null,
