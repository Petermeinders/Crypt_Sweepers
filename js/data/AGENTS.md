...
- **Enemy `spawn` field shapes:**
  ...
  - `{ voidTrial: true, voidMinTier?, voidMinFloor? }` — void trial mob pool; mechanics in `js/systems/VoidEnemyMechanics.js`
  - `{ voidTrial: true, voidBoss: true }` — void trial finale only (`VOID_TRIAL_BOSS_ID` → `void_overseer` via `TileEngine._pickEnemyType`)
- **Enemy `behaviour` field** controls spawn routing in `TileEngine`: `'standard'`, `'boss'`, `'archer'` (archer is never spawned via normal pool — only via `_spawnArcherGoblin()`). Fast ambush enemies use `attributes: ['fast']` instead of a separate tile type.
