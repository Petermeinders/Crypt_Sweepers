...
- **Parry arc canvas is 300×300, offset -20 px.** `.parry-arc-canvas` is `top: -20px; left: -20px; width: 300px; height: 300px` to centre a 300 px canvas over the 260 px arena. The extra 20 px of canvas on each side prevents the glow stroke (`lineWidth: 20`) from being clipped at the canvas boundary. `transform-origin: center` still scales from the correct arena centre.
